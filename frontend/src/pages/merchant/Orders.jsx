...
import { ArrowLeft, MapPin, Phone, PackageSearch, Search, X, CheckCircle2, Circle, User } from "lucide-react";
import { COLORS } from "../../constants/colors";
import { Card, Badge, Button, EmptyState, SkeletonCard, Pagination, Drawer } from "../../components/ui";
import { ORDER_STATUS_VARIANT, ORDER_FLOW } from "../../utils/orderStatus";

const STATUS_FILTERS = ["ALL", "CONFIRMED", "SHIPPED", "OUT_FOR_DELIVERY", "DELIVERED"];
...
function MerchantOrders() {
    ...
    const updateStatus = async (orderId, status) => {
        try {
            await API.put(`/merchant/orders/${orderId}/status`, null, { params: { status } });
            await fetchOrders();
    ...
    const stats = [
        { label: "Total orders", value: orders.length, color: "text-ink-900" },
        { label: "Confirmed", value: orders.filter((o) => o.status === "CONFIRMED").length, color: "text-brand-600" },
        { label: "Shipped", value: orders.filter((o) => o.status === "SHIPPED").length, color: "text-warning-600" },
        { label: "Delivered", value: orders.filter((o) => o.status === "DELIVERED").length, color: "text-success-600" },
        {
            label: "Revenue",
            value: `₹${orders.reduce((sum, o) => sum + o.totalPrice, 0).toLocaleString("en-IN")}`,
    ...
    const filteredOrders = orders.filter((o) => {
        ...
        const matchesSearch = !q || String(o.orderId).includes(q) || o.items?.some((i) => i.productName?.toLowerCase().includes(q));
    ...
    return (
        <div className="mx-auto max-w-6xl">
            ...
            <h1 className="mb-6 text-2xl font-bold text-ink-950 sm:text-3xl">Merchant Orders</h1>
            ...
            <div className="mb-8 grid grid-cols-2 gap-4 md:grid-cols-5">
                {stats.map((s) => (
                    <Card key={s.label} padding="sm">
                        ...
                        <h2 className={`mt-1 text-2xl font-bold ${s.color}`}>{s.value}</h2>
                    </Card>
                ))}
            </div>
            ...
            {loading ? (
                ...
                <EmptyState
                    ...
                    description={orders.length === 0 ? "Orders for your products will appear here." : "Try a different search or filter."}
                />
            ) : (
                    ...
                    {paginatedOrders.map((o) => (
                        ...
                        >
                            <div className="flex items-start justify-between gap-2">
                                <div>
                                    <h2 className="text-lg font-bold text-ink-900">Order #{o.orderId}</h2>
                                    <p className="mt-0.5 text-sm text-ink-500">{o.items?.length || 0} items</p>
                                </div>
                            ...
                            <div className="mt-3 flex -space-x-2.5">
                                ...
                                {o.items?.length > 4 && (
                                    <span className="flex h-10 w-10 items-center justify-center rounded-lg border-2 border-white bg-ink-100 text-xs font-semibold text-ink-500">
                                        +{o.items.length - 4}
                                    </span>
                                )}
                            </div>
                            ...
                            <div className="mt-3 flex items-center justify-between">
                                <span className="text-lg font-bold text-success-600">₹{Number(o.totalPrice).toLocaleString("en-IN")}</span>
                ...
                {selectedOrder && (
                    <div className="space-y-6">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="text-xl font-bold text-success-600">₹{Number(selectedOrder.totalPrice).toLocaleString("en-IN")}</span>
                        </div>
                                        ...
                                        return (
                                            <div key={step} className="flex flex-1 items-center last:flex-none">
                                                <div className="flex flex-col items-center gap-1.5">
                                                    ...
                                                    <span className={`text-center text-[10px] font-medium ${done ? "text-success-700" : "text-ink-400"}`}>
                                                        {step.replaceAll("_", " ")}
                                                    </span>
                                                </div>
                                                {i < ORDER_FLOW.length - 1 && (
                                                    <div className={`mx-1 h-0.5 flex-1 ${i < currentIndex ? "bg-success-500" : "bg-ink-200"}`} />
                                                )}
                                            </div>
                                        );
                                    })}
                        ...
                        <div className="rounded-xl border border-ink-100 bg-ink-50 p-4">
                            <h3 className="mb-2.5 flex items-center gap-2 text-sm font-semibold text-ink-800">
                                <User size={14} className="text-brand-600" /> Customer & delivery
                            </h3>
                            <div className="space-y-1 text-sm text-ink-600">
                                <p className="flex items-center gap-1.5 font-medium text-ink-800"><MapPin size={13} className="text-ink-400" /> {selectedOrder.address?.addressType || "Address"}</p>
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                {selectedOrder.items?.map((item, i) => {
                                    const colorObj = COLORS.find((c) => c.name === item.selectedColor);
                                    return (
                                        <div key={i} className="flex items-center gap-3 rounded-xl border border-ink-100 p-3">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <span className="text-sm font-bold text-success-600">₹{Number(item.price).toLocaleString("en-IN")}</span>
                                            </div>
                                        </div>
                                    );
                                })}
                        ...
                        {selectedOrder.status === "CONFIRMED" && (
                            <Button fullWidth onClick={() => updateStatus(selectedOrder.orderId, "SHIPPED")}>Mark as shipped</Button>
                        )}
                    </div>
                )}
            </Drawer>
